...
Before/after and current QA captures are generated from the real Godot scene into ignored `art_reviews/baseline/` and `art_reviews/current/` directories. They are intentionally local because the selected development models are proprietary references.

## Assets

The local development build can load owner-supplied character and prop references from ignored `assets/dev_local/` paths described by the committed metadata-only registry `assets/local_asset_manifest.json`. Those binaries are not CC0 and are not distributable StudyTown assets. A clean clone automatically uses procedural characters and CC0 Kenney prop fallbacks. See `ASSET_LICENSES.md`, `ASSET_PIPELINE.md`, `docs/ASSET_AUDIT.md`, and `docs/LOCAL_ASSET_IMPORT.md`.
