...
const SeatAvailabilityGlowScript := preload("res://scripts/study/seat_availability_glow.gd")
...
var session_focus_text := ""
var session_book_name := ""
...
func _process(delta: float) -> void:
	...
	if is_instance_valid(menu_character):
		var menu_base_yaw := float(
			menu_character.get_meta(
				"menu_base_yaw",
				0.0
			)
		)
		menu_character.rotation.y = (
			menu_base_yaw
...
func _begin_resting(_spot_index: int) -> void:
	application_flow.start_break()
...
func _ft_make_session_setup_camera(spot) -> Camera3D:
	...
	var target: Vector3 = spot.sitting_position + Vector3.UP * 1.62
	var chosen: Vector3 = target + forward * 4.0 + Vector3.UP * 0.8
	var found := false
	# Never choose a rear fallback. Try both front-quarter sides, then elevation.
	for front_distance in [4.0, 3.2, 2.4, 1.6, 0.9]:
		for height in [0.8, 1.25, 1.8]:
			for side in [0.0, 2.8, -2.8, 2.0, -2.0, 1.2, -1.2, 3.6, -3.6]:
				var candidate: Vector3 = target + forward * front_distance + right * side + Vector3.UP * height
		...
		if found:
			break
	var camera_right := (target - chosen).normalized().cross(Vector3.UP).normalized()
	# Offset the look target to place the player on the right of the setup panel.
	var composition_target := target - camera_right * 1.42
	var camera := _make_camera(chosen, composition_target, 43.0, false)
	camera.set_meta("shot_name", "session setup front quarter")
	camera.set_meta("requires_player_visibility", true)
	camera.set_meta("visibility_validated", found)
	return camera

func _ft_build_seated_session_panel(_spot_index: int) -> void:
	application_flow.navigate(application_flow.State.SESSION_SETUP)

func _begin_focus(_spot_index: int) -> void:
	application_flow.start_session()

func _transition_player_to_study_spot(spot) -> void:
	_set_movement_enabled(false)
	player.velocity = Vector3.ZERO

	character_loader.play_animation(player_visual, "Walk", 0.1)
	# ------------------------------------------------------------
...
func _build_focus_hud(_task: String) -> void:
...
func _prepare_focus_camera_pool(
	...
	# Tight window-facing seats need shallower front-quarter offsets.
	offsets.append_array([
		Vector3(2.8, 2.8, 1.6), Vector3(-2.8, 2.8, 1.6),
		Vector3(3.6, 3.1, 0.9), Vector3(-3.6, 3.1, 0.9),
		Vector3(2.0, 3.2, 2.4), Vector3(-2.0, 3.2, 2.4),
	])
	var basis := Basis(
		Vector3.UP,
		spot.facing_yaw
...
func _is_focus_shot_clear(camera_position: Vector3, spot) -> bool:
	...
	# NPC render meshes do not use the world-collision layer. Check their
	# silhouettes explicitly so an otherwise clear shot cannot sit in a head.
	for npc in npcs:
		if not is_instance_valid(npc):
			continue
		var visual_bounds := _combined_world_aabb(npc)
		if not visual_bounds.valid:
			continue
		var bounds_box: AABB = visual_bounds.aabb
		# Imported skinned meshes report their small bind-pose bounds here,
		# not their animated silhouette. Include a profile-sized body envelope.
		var silhouette_height := 2.9
		if is_instance_valid(npc.visual):
			var profile = npc.visual.get_meta("character_profile", null)
			if profile != null:
				silhouette_height = profile.label_height
		bounds_box = bounds_box.merge(AABB(npc.global_position + Vector3(-0.68, 0.0, -0.68), Vector3(1.36, silhouette_height, 1.36)))
		bounds_box = bounds_box.grow(0.08)
		if bounds_box.has_point(camera_position):
			return false
		for point: Vector3 in target_points:
			if bounds_box.intersects_segment(camera_position, point) != null:
				return false
